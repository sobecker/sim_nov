...
def run_hybrid_exp(params_exp,params_mb, params_mf,verbose=True,saveData=False,returnData=False,dirData='',rec_init=True):
    ...
    mb_rec = nor.surnor_recorder(all_params_mb,params_mb['rec_type']) 
    ...
    for trial in range(params_exp['number_trials']):
        ...
        for e in range(params_exp['number_epi']):
            ...
            while not foundTerminal and mb_t<Tmax:
                ...
                if s_new in np.nonzero(env.R)[0]:
                    foundGoal = True
                    if verbose: print(f"Found goal state after {mb_t} iterations.\n") 
                if s_new == term:
                    foundTerminal = True
    ...
    if returnData:
        ...
        # Read out MB data
        mb_data_basic = mb_rec.readoutData_basic()
        mb_all_data = [mb_data_basic]
        if mb_rec.rec_type=='advanced':
            mb_q = mb_rec.readoutData_advanced()      
            mb_all_data.append(mb_q) 
        end_data = timeit.default_timer()
        # Read out MF data
        mf_data_basic = mf_rec.readoutData_basic()
        mf_exp_data = [mf_data_basic]
        if mf_rec.rec_type=='advanced2':
            mf_wc,mf_ec,mf_wa,mf_ea = mf_rec.readoutData_advanced2()
            mf_exp_data.append(mf_wc)
            mf_exp_data.append(mf_ec)            
            mf_exp_data.append(mf_wa)
            mf_exp_data.append(mf_ea)  
    ...
    if saveData:
        ...
        mb_rec.saveData(dataFolder,format_data='df',data_name='mb_data_basic')
        mb_rec.saveData(dataFolder,format_data='csv',data_name='mb_data_basic')
        # Save MF params + data
        mf_rec.saveParams(params_mf,dataFolder,'dict',params_name='mf_params')
        mf_rec.saveParams(params_mf,dataFolder,'csv',params_name='mf_params')
        mf_rec.saveData(dataFolder,'df',data_name='mf_data_basic')
        mf_rec.saveData(dataFolder,'csv',data_name='mf_data_basic')
        end_data = timeit.default_timer()
