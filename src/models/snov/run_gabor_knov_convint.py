...
import src.models.snov.run_gabor_knov_patches as rgkp
import src.models.snov.run_gabor_knov2 as rgk2
import src.models.snov.gabor_stimuli as gs

### This file contains functions to simulate Gabor novelty with convolutional kernels that integrate per image patch ###

####################################################################################################################################################################
# Initialize Gabor novelty and return dictionary with parameters
def init_gabor_knov(pnum=(2,4),psize=None,image_dim=(100,200),gnum=16,k_type='triangle',ksig=1,kcenter=1,cdens=2,seed=12345,rng=None,mask=True,conv=False,parallel=False,adj_w=True,adj_f=False,sampling='basic',contrast='off',softmax_norm=True,eps_k=1,alph_k=0.1,add_empty=False,debug=False,convint=True):
    
    # Define image patches (by indices)
    if psize is None: 
        psize = (int(np.ceil(image_dim[0]/pnum[0])),int(np.ceil(image_dim[1]/pnum[1])))
    pidx = rgkp.define_patches(image_dim,psize,pnum)
    ...
    for i in range(len(kcl_df)):
        ...
        kgabor.append(gi)
        if mask and not (conv or convint):
...
####################################################################################################################################################################
# Adaptive lr
def sim_knov_gabor(pidx,ptot,stim,k,ksig0,eps_k,t0_update=0,cdens=2,conv=False,convint=True,plot=False,figsize=None,rec_time=100,save_plot=False,save_plot_dir='',save_plot_name='',idx=True,parallel_k=False,flip=False,flip_idx=None,kmat_seq_flipped=None):
    if idx:
        stim_unique = stim[0]
        stim_idx    = stim[1]
    else:
    ...
    kwl = []; kmat_seq = []
    knum = len(k) * len(pidx)
    kwl, _, kmat_seq, _, _, _ = knov_vec.init_convint_nov(k,ksig0,seq=stim_unique,parallel=parallel_k,pidx=pidx)
    ...
    if not represented:
        ...
        for t in range(len(stim_idx)):
            ...
            rk_new    = knov_vec.update_rk_approx1(kwl,kmat_seq[:,stim_idx[t],:].reshape((-1,1)))             # Update responsibilities
            # Test
...
# Fixed lr
def sim_knov_gabor_flr(pidx,ptot,stim,k,ksig0,alph_k,t0_update=0,cdens=2,convint=True,plot=False,figsize=None,rec_time=100,save_plot=False,save_plot_dir='',save_plot_name='',idx=True,parallel_k=False,flip=False,flip_idx=None,kmat_seq_flipped=None):
    if idx:
        stim_unique = stim[0]
        stim_idx    = stim[1]
    else:
...
def run_gabor_knov_withparams(stim,k_params,plot_kernels=False,save_plot=False,save_plot_dir='',save_plot_name='',idx=True,parallel_k=False,flip=False,kmat_seq_flipped=None):
    ...
    klist_gabor,plist_gabor,nlist_gabor,kwlist_gabor = sim_knov_gabor(pidx=k_params['pidx'],
                                                                      ptot=k_params['pnum'][0]*k_params['pnum'][1],
                                                                        ...
                                                                        convint=True if 'cdens' in k_params.keys() else False,
                                                                        plot=plot_kernels,
                                                                        rec_time=20,
                                                                        save_plot=save_plot,
