...
source_set = [1,2,5,6]
ktype_set = ['box' if s in [1,4] else 'tanh' if s in [2,5] else 'triangle' for s in source_set]
target_set = 7
base_path_data = f'/Volumes/lcncluster/becker/RL_reward_novelty/data/2025_07_robustness2_width/'
# base_path_data = f'/Volumes/lcncluster/becker/RL_reward_novelty/data/2025_07_robustness_shape_width/'
source_paths   = [os.path.join(base_path_data,f'{model_name}_set{source}_sep/combined_data/') for source in source_set]
target_path    = os.path.join(base_path_data,f'{model_name}_set{target_set}_sep/combined_data/')
...
for ii, source in enumerate(source_set):

    # Load grid and shift grid_ids
    grid_ii = pd.read_csv(os.path.join(source_paths[ii],'grid.csv'))
    min_idx = grid_ii['grid_id'].min()
    max_idx = grid_ii['grid_id'].max()
    grid_ii['grid_id_old2'] = grid_ii['grid_id'].copy()
    grid_ii['grid_id'] = grid_ii['grid_id'] - min_idx + grid_cc
    grid_ii['k_type'] = ktype_set[ii]
    grid_all.append(grid_ii)

    # Load done file and shift grid_ids
    with open(os.path.join(source_paths[ii],'done.txt'),'r') as f:
    ...
    for jj in grid_ii['grid_id_old2'].values:
        os.rename(os.path.join(source_paths[ii],f'grid_{jj}'),os.path.join(target_path,f'grid_{jj-min_idx+grid_cc}'))
