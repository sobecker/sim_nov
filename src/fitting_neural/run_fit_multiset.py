...
model      = 'snov-complex-leaky'     # 'cnov_leaky', 'cnov_fr' or 'snov-complex-leaky', 'snov-complex-fr'
...
if comp_type=='fit':    
    ...
    sampling_type = 'normal'     # 'normal','bootstrap','jackknife','loo','jackknife-loo'
    n_bootstrap   = 50
...
if 'cnov' in model:
    ...
    set_num        = [9,10] if 'leaky' in model else [7,8] # 5 for leaky, 7 for fr
    save_path_base = '/lcncluster/becker/RL_reward_novelty/data/' if run_from_cluster else '/Volumes/lcncluster/becker/RL_reward_novelty/data/'
    name_proj      = '2025_05_grid_search_new'
    name_set       = [f'{model}_set{sn}_sep' for sn in set_num]
    load_path      = [save_path_base + f'{name_proj}/{nset}/combined_data/' for nset in name_set]
    ...
    try: 
        ...
        if comp_type=='fit':
            ...
            if sampling_type=='normal':
                ...
                jack_metrics = fit_homann.crossvalidation_homann_exp(sim_data,homann_data,coef_steady=True,sampling_type=sampling_type,weighting=weighting,drop_type=drop_type,combine_m=False,save_path=save_path_i)

                if 'jackknife' in sampling_type:
                    for i_jm, jm in enumerate(['jack_mse_mean', 'jack_mse_std', 'jack_mse_se']):
                        grid_df.loc[grid_df.grid_id_multi==i, jm] = jack_metrics[i_jm]
