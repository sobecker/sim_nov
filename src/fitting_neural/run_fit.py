...
comp_type  = 'fit'            # 'fit' or 'corr'
model      = 'cnov_leaky'     # 'cnov_leaky', 'cnov_fr' or 'snov-complex-leaky', 'snov-complex-fr'
nonleaky   = True if 'leaky' in model else False
robustness = False # whether to fit robustness simulations (TRUE) or regular simulations (FALSE)

if comp_type=='fit':    
    weighting     = 'equal-exp'         # 'equal-samples', 'equal-exp' or 'none' [not available yet for bootstrap!]
    drop_type     = 'exp'                  # 'sample' or 'exp' (drop one sample or one experiment - only has effect if 'jackknife' or 'loo' in sampling_type); recommended to choose drop consistent with weighting!
    sampling_type = 'normal'        # 'normal','bootstrap','jackknife','loo','jackknife-loo', 'outerjack-cv' / 'outerboot-cv'
    n_bootstrap   = 50
    refit         = True  # only applies to sampling_type='loo', 'outerjack-cv', or 'outerboot-cv'
    ...
    sampling_type = 'normal'          # 'normal', bootstrap'
    n_bootstrap   = 50
...
if 'cnov' in model:
    ...
    set_num        = 9 if 'leaky' in model else 7 # 5 for leaky, 7 for fr
    save_path_base = '/lcncluster/becker/RL_reward_novelty/data/' if run_from_cluster else '/Volumes/lcncluster/becker/RL_reward_novelty/data/'
    name_proj      = '2025_05_grid_search_new'
    name_set       = f'{model}_set{set_num}_sep' 
    save_path      = save_path_base + f'{name_proj}/{name_set}/combined_data/'
...
str_nonleaky   = 'nonleaky_' if nonleaky else ''
...
if refit:
    ...
    if nonleaky:
        grid_orig = grid_orig[grid_orig['alph_leak']==0]
            ...
            for j, nn in enumerate(data_names):
                if 'cnov' in model:
                    ...
                    if sampling_type=='bootstrap':
                        ...
                        boot_stats_nn_df.to_csv(save_path_i+f'{str_nonleaky}boot_stats_{nn}.csv',index=False)
                        boot_stats.append(boot_stats_nn)

                    stats_nn = data_nn[[data_var[j],data_val[j]]].groupby(data_var[j]).mean().reset_index()
                    stats_nn.to_csv(save_path_i+f'{str_nonleaky}stats_{nn}.csv',index=False)
