...
import src.scripts.gabor_kernels.run_gabor_knov as gknov
import src.scripts.gabor_kernels.run_gabor_knov2 as gknov2
...
def get_trans_response(data,aggname,len_seq=3,window_size=3,get_stats=True,steady=False):
        data    = data.reset_index(names='t')   # save time step per trial as separate column and reset index
        data    = data.reset_index(names='T')   # save global time step as separate column and reset index
        data    = data.loc[data.stim_type!='nov'].reset_index(drop=True)  # remove novel sequences
        loc_nov = list(set(np.where(data.stim_type.values=='fam_r')[0]) & set(np.where(data.stim_type.values=='fam')[0]+1))  # get onset time of familiar probe (after recovery period)
        loc_nov.sort()
        ...
        for i in range(0,len(loc_nov)):                                                     # add steady state
            if i==len(loc_nov)-1:   
                data.loc[loc_nov[i]:,'steady'] = steady_nov[i]
                data.loc[loc_nov[i]:,'tr']     = tr_nov[i]   
                # data['steady'].iloc[loc_nov[i]:] = steady_nov[i]
                # data['tr'].iloc[loc_nov[i]:]     = tr_nov[i]      
            else:                   
                data.loc[loc_nov[i]:loc_nov[i+1]-1,'steady'] = steady_nov[i] 
                data.loc[loc_nov[i]:loc_nov[i+1]-1,'tr']     = tr_nov[i] 
