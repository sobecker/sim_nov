...
def sim_tau_emerge(ivec,iseed,itype,k_params,n_fam,idx=True,norm=True,flr=False,verbose=False,kwargs={}):
    stop_nokernels = kwargs['stop_nokernels'] if 'stop_nokernels' in kwargs.keys() else True
    # Simulate model
    all_data = []
    for j in range(len(ivec)):
        if flr:
            idata,_,_ = gknov2.run_gabor_knov_withparams_flr(ivec[j],k_params,idx=idx,stop_nokernels=stop_nokernels)
        else:
            idata,_,_ = gknov2.run_gabor_knov_withparams(ivec[j],k_params,idx=idx,stop_nokernels=stop_nokernels)
    ...
    if 'savepath_sim_data' in kwargs.keys() and 'savename_sim_data' in kwargs.keys():
        data.to_csv(os.path.join(kwargs['savepath_sim_data'],kwargs['savename_sim_data']+'_l.csv'))
...
def sim_tau_memory(ivec,iseed,itype,k_params,n_images,idx=True,flr=False,flip=False,kmat_seq_flipped=None,verbose=False,kwargs={}):
    ...
    for j in range(len(ivec)):
        ...
        if flr:
            idata,_,_ = gknov2.run_gabor_knov_withparams_flr(ivec[j],k_params,idx=idx,flip=flip,kmat_seq_flipped=kmatj,stop_nokernels=stop_nokernels)
        else:
            idata,_,_ = gknov2.run_gabor_knov_withparams(ivec[j],k_params,idx=idx,flip=flip,kmat_seq_flipped=kmatj,stop_nokernels=stop_nokernels)
    ...
    if 'savepath_sim_data' in kwargs.keys() and 'savename_sim_data' in kwargs.keys():
        data.to_csv(os.path.join(kwargs['savepath_sim_data'],kwargs['savename_sim_data']+'_m.csv'))
...
def sim_tau_recovery(ivec,iseed,itype,k_params,dN,idx=True,flr=False,verbose=False,kwargs={}):
    ...
    if 'savepath_sim_data' in kwargs.keys() and 'savename_sim_data' in kwargs.keys():
        data.to_csv(os.path.join(kwargs['savepath_sim_data'],kwargs['savename_sim_data']+'_lp.csv'))
...
def run_homann_exp_n(k_params,params_input,inputs,parallel=False,save_path='',kwargs={}):
    ...
    # Initialize Gabor kernel novelty model ## DIFFERENT FOR COMPLEX CELLS
    k_params_init = gknov2.init_gabor_knov(gnum=k_params['knum'],
                                    ...
                                    add_empty=kwargs['add_empty'] if 'add_empty' in kwargs.keys() else False)
...
def fit_homann_exp(sim_data,homann_data,coef_steady=True,regr_meas='score',save_path='',save_name=''):
    ...
    if len(save_path)>0:
        ...
        for i,n in zip(range(len(pred_data)),data_names):
            pd.DataFrame({data_var[i]:pred_data[i][0],data_val[i]:pred_data[i][1]}).to_csv(save_path+f'pred_{n}{save_name}.csv',index=False)
...
def generate_input(params_input,num_sim,init_seed=12345,input_corrected=True,input_sequence_mode='sep'):
    ...
    inputs = []
    if input_corrected:
        # Create input sequences for experiments (new scripts, corrected experimental protocols)
        ...
        # Create input sequences for experiments (old scripts)
        for i in range(len(seed_input)):
...
def simulate_grid_point(i,k_params_ext,k_params_def,grid_df,grid_keys,save_path,params_input,inputs,parallel_exp,comp_fit,comp_corr,cluster,kwargs_def,kwargs_ext):
    ...
    # k_params_i = grid_df[grid_keys].iloc[i].to_dict()  
...
if __name__=="__main__":
    ...
    if args.config_file:
        ...
        config = json.load(open('./src/scripts/gabor_kernels/grid_search_manual/configs_set1_simple_cells_seqsep/simple_cells_triangle_adj_w3_G-40.json'))
        # config = json.load(open('./src/scripts/gabor_kernels/grid_search_manual/configs_set2_simple_cells/simple_cells_triangle_adj_w3_G-40.json'))
    print(config)
