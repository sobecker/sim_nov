...
def run_optsim_nac_tree(res,agent_num,epi_num,base_params,sim_name,plot_title='',rec_type='advanced2',dir_data='',dir_fig='',seeds=None):
    ...
    # Update novelty parameters
    nov_params = ['eps_leak', 'alph_leak', 'k_alph']  
    for novp in nov_params:
        if novp in res_params.keys():
            sim_params['h'][novp] = res_params[novp]
    if 'eps_leak' in sim_params['h'].keys():
        sim_params['h']['eps_leak'] = [sim_params['h']['eps_leak']] 
    if 'update_type' in base_params['h'].keys():
        sim_params['h']['update_type'] = base_params['h']['update_type']
...
def run_optsim_hybrid_tree(res,agent_num,epi_num,base_params,sim_name,plot_title='',rec_type='advanced2',dir_data='',dir_fig='',seeds=None):
    ...
    for novp in nov_params:
        if novp in res_params.keys():
            sim_params['mb_h'][novp] = res_params[novp]
            sim_params['mf_h'][novp] = res_params[novp] 
    if 'eps_leak' in sim_params['mb_h'].keys():
        sim_params['mb_h']['eps_leak'] = [sim_params['mb_h']['eps_leak']] 
    if 'eps_leak' in sim_params['mf_h'].keys():
        sim_params['mf_h']['eps_leak'] = [sim_params['mf_h']['eps_leak']] 
    if 'update_type' in base_params['mb_h'].keys():
        sim_params['mb_h']['update_type'] = base_params['mb_h']['update_type']
    if 'update_type' in base_params['mf_h'].keys():
        sim_params['mf_h']['update_type'] = base_params['mf_h']['update_type']
