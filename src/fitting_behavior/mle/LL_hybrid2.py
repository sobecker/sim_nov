...
def ll_hybrid2(params,data,mb_qvals=[],mf_qvals=[],verbose=False,start_ll=None,stop_ll=None):
    ...
    try:
        ...
        # Extract params MB
        if not 'mb_ntype' in params.keys():  
            params['mb_ntype'] = 'N'
        if not 'mb_k' in params.keys():      
            params['mb_k']     = 0
        ...
        for trial in range(params['number_trials']):
            ...
            # Set random seed
            np.random.seed(params['seeds'][trial])  

            # Initialize MF agent
            params_mf = params.copy() 
            overlap = ['mf_rec_type','mf_k','mf_ntype','mf_h','mf_w']
            ...
            if mb_hierarchical:  
                ...
                mb_update_type  = params_mb['h']['update_type'] if 'update_type' in params_mb['h'].keys() else 'var'
            ...
            for e in range(params['number_epi']):
                ...
                for it in range(1,len(data.loc[data.epi==e])):
                    ...
                    a    = int(data['action'].values[it-1]) # get action from data
                    lpa  = np.log(p_softmax[list(not_nan[0]).index(a)]) 

                    if it>=start_ll and it<stop_ll:
                        ll  += lpa  

                    s_new_env, r    = env.evalAction(a,s)
                    s_new           = data['next_state'].values[it-1]
                    ...
                    if len(mb_qvals)>0:
                        ...
                        if not (mb_qN[qvals_notnan]==mb_qvals['qvals'].values[it-1][qvals_notnan]).all():
                            all_q_mb_equal = False
                    if len(mf_qvals)>0:
                        qvals_notnan = (~np.isnan(mf_new_was)).nonzero()
                        if not (np.array(mf_new_was)[qvals_notnan]==mf_qvals[it-1]).all():
                    ...
                    # Update statefor next iteration
                    s = s_new
    except Exception as e:
        if verbose: print(f"Error occurred: {e}. Returning infinite LL.")
        ll = -np.inf
        all_q_mb_equal = None 
        all_q_mf_equal = None
