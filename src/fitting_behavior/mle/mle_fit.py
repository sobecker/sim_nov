...
def run_mle_fit(config):
    ...
    if fit_any: 
        ...
        update_type = 'leaky' if ('leaky' in alg_type_fit) else 'fixed' if ('fixed' in alg_type_fit) else 'var'

        if 'multinov' in alg_type_fit: # combine two novelty distributions (make of box / triangle components with / without center)
        ...
        elif 'placefields' in alg_type_fit: # use place field distribution
        ...
        filter_duplicates   = 'noduplicates' in alg_type_fit
        maze_norm           = 'mazenorm' in alg_type_fit
        comp_norm           = False if 'nocompnorm' in alg_type_fit else True

        if 'nor' in alg_type_fit.replace('mazenorm','').replace('nocompnorm',''): 
            if 'hnor' in alg_type_fit:
                params = bpo.baseparams_h1mbnor_eps1(level,join_levels=join_levels,notrace=notrace,center=center,center_type=center_type,update_type=update_type,placefields=placefields,filter_duplicates=filter_duplicates,maze_norm=maze_norm,comp_norm=comp_norm)
            else:
            ...
            if 'eps1' in alg_type_fit:
                params['h']['eps_leak'] = [1] # non-transformed default value
            if 'nonleaky' in alg_type_fit:
                params['h']['alph_leak'] = 0

        elif 'nac' in alg_type_fit:
            if 'hnac' in alg_type_fit:
                params = bpo.baseparams_h1nac_eps1(level,join_levels=join_levels,notrace=notrace,center=center,center_type=center_type,update_type=update_type,filter_duplicates=filter_duplicates,maze_norm=maze_norm,comp_norm=comp_norm)
            else: 
            ...
            if 'hhybrid' in alg_type_fit:
                ...
                params = bpo.baseparams_all_hhybrid_comb(mb_type,mf_type,levels=level,join_levels=join_levels,notrace=notrace,center=center,center_type=center_type,path_surnor= base_path / 'src' / 'models' / 'mb_agent',update_type=update_type,filter_duplicates=filter_duplicates,maze_norm=maze_norm,comp_norm=comp_norm)
            ...
            if 'eps1' in alg_type_fit:
                params['mb_h']['eps_leak'] = [1] # non-transformed default value
                params['mf_h']['eps_leak'] = [1] # non-transformed default value
            if 'nonleaky' in alg_type_fit:
        ...
        ## Compute MLE with multiple random starts (if specified) ##############################################################################
        ...
        ## Fit both separately and jointly #####################################################################################################
        if comb_type=='':
            ...
            print(dir_save)
            res.to_csv(dir_save / f'{save_name}.csv')
            res.to_pickle(dir_save / f'{save_name}.pickle')
            save_name = f'{save_folder}_app'
            resapp.to_csv(dir_save / f'{save_name}.csv')
            resapp.to_pickle(dir_save / f'{save_name}.pickle')

        ## Fit separately or jointly (based on comb_type) ######################################################################################
        else:
