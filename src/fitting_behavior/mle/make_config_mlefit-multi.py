...
alg_type       = [f'{leakiness_type}{multinov_type}_hnor_notrace'] 
levels         = [[1,6], [2,6], [3,6], [4,6], [5,6]] # level 1: component center is the second branching point, level 6: component centers are the leaf nodes
...
for i in range(len(alg_type)):
    ...
    for j in range(len(levels)):
        ...
        params = {'data_type':          data_type,
                    ...
                    'local':            local,
                    'level':            levels[j],
                    'save_path':        str(sl.get_rootpath() / 'data' / 'mle_results' / f'fits_{"multi-start/" if randstart else "single-run"}' / f'mle_{alg_type[i]}{single_run_id}')}

        path = sl.get_rootpath() / 'src' / 'fitting_behavior' / 'mle' / 'mle_fit_configs'
        sl.make_long_dir(path)
        name = f'{params["save_name"]}_{comb_type}'
