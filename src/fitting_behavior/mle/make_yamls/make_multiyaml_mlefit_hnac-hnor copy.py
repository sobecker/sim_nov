...
opt_alg     = ['Nelder-Mead']
comb_type   = ['app']
single_run_id = '_ri5' # '', '_1', '_2', ... to distinguish multiple single runs with different initial conditions
    ...
    for oo in range(len(opt_alg)):
        for cc in range(len(comb_type)):
            ...
            for ll in range(len(levels)):
                ...
                full_job_name = f"{save_name1}-l{levels[ll]}-{comb_type[cc]}{single_run_id}".replace('_','-').lower()
