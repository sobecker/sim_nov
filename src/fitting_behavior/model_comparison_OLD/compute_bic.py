...
def compute_bic(path_models,candidates,opt_method,comb_type,path_save,name_save,maxit,n_app=None,n_sep=None):
    ...
    for i in range(len(candidates)):
        ...
        path = path_models / f'mle_{candidates[i]}'
        file = f'mle_{candidates[i]}_{comb_type}.csv'
...
if __name__=="__main__":
    ...
    # alg_types    = [f'{leakiness_type}_hnor_notrace']
    # multicomp    = [True]*len(alg_types) # whether multiple levels of components are used in the same distribution (True)
    # single_run_id = '_1' # '', '_1', '_2'
    
    ## Single-level components (no compnorm, no mazenorm)
    alg_types    = [f'{leakiness_type}_hnor_notrace']
    multicomp    = [False]*len(alg_types) # whether multiple levels of components are used in the same distribution (True)
    single_run_id = '_1' # '', '_1', '_2'

    data_type    = 'mice' # 'mice','opt','naive'
    opt_method   = 'Nelder-Mead' # 'Nelder-Mead','L-BFGS-B','SLSQP'
    comb_types   = 'app'
    measure_type = ['LL','bic']     # 'bic','LL'
    epss         = [[50,100]]*len(alg_types)
    maxit        = [False]*len(alg_types) #True

    base_path    = sl.get_rootpath() / 'data' / 'mle_results' / 'fits_single-run'
    
    for i_alg in range(len(alg_types)):
        for i_type in range(len(measure_type)):

            alg_type = alg_types[i_alg]
            comb_type = comb_types
            eps = epss[i_alg]
            
            if multicomp[i_alg]:
                path_models = base_path / f'{alg_type}{single_run_id}'
                levels      = [[1,6], [2,6], [3,6], [4,6], [5,6]]
                candidates  = [f'{alg_type}-l{"-".join(map(str,i))}' for i in levels]
                xtl         = [f'l{"-".join(map(str,i))}' for i in levels]
            else:
                path_models = base_path / f'mle_{alg_type}{single_run_id}'
                levels      = [1,3,4,5,6] 
                candidates  = [f'{alg_type}-l{i}' for i in levels]
                xtl         = [f'l{i}' for i in levels]

            path_save   = base_path / 'data' / 'model_selection' / 'bic'
            sl.make_long_dir(path_save)
            path_save1  = base_path / 'output' / 'model_selection' / 'bic'
            sl.make_long_dir(path_save1)

            name        = ('multi' if multicomp[i_alg] else '') + f'{alg_type}' + single_run_id
            name_save   = ('multi' if multicomp[i_alg] else '') + f'{alg_type}' + single_run_id
            name_save1  = ('multi' if multicomp[i_alg] else '') + f'{alg_type}_which-alg' + single_run_id
